...
class Events1:
    ...
    def general(self, event): #-----------------------------------------general
        ...
        if event == 'get_pos':            
            joint_list, cartesian_position = self.robot1.get_position()
        ...
        if self.set_home:
            joint_list, cartesian_position = self.robot1.get_position()
            if event == 'HOME': self.robot1.main_positions_list[0] = joint_list
            if event == 'pos1': self.robot1.main_positions_list[1] = joint_list
            if event == 'pos2': self.robot1.main_positions_list[2] = joint_list
            if event == 'pos3': self.robot1.main_positions_list[3] = joint_list
            if event == 'pos4': self.robot1.main_positions_list[4] = joint_list
            if event == 'pos5': self.robot1.main_positions_list[5] = joint_list
    ...
    def advanced_functions(self, event, data = None):
        ...
        if event == "update_tool":            
            self.robot1.tool_dimensions = data
            self.robot1.add_tool_fun()
        
        if event =="get_tool_dim":
            return self.robot1.tool_dimensions
        ...
        if event == "import":
            ...
            with open(pik_file, "rb") as f:
                try:
                    ...
                    self.robot1.main_positions_list = list_data[3]
                    [self.robot1.table_position, self.robot1.table_dimensions] = list_data[4]
            ...
            with open(pik_file, "wb") as f:
                ...
                list_data = [self.robot1.Joint_points, self.robot1.Cartesian_points, self.robot1.mov_type, self.robot1.main_positions_list, table_data]
